...
func TestState(t *testing.T) {
	...
	for _, s := range []State{Error, NaN, DivisionByZero, Overflow, Underflow, NegativeInUnsignedOp, NotEnoughBytes, InvalidFormat, ScaleOutOfRange, RescaleToLowerScale, SqrtNegative, Null, Inexact, InvalidRoundingMode, InvalidLossPolicy, NotConverged, DomainError} {
		if s.IsOK() {
...
func TestIsValid(t *testing.T) {
	for s := State(0); s <= DomainError; s++ {
		if !s.IsValid() {
			t.Errorf("state %d must be valid", s)
		}
	}
	if (DomainError + 1).IsValid() || State(255).IsValid() {
